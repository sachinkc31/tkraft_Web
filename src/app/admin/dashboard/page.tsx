...
export default function AdminDashboardPage() {
  ...
  // Load CMS data on tab shift
  useEffect(() => {
    const savedKey = localStorage.getItem("tkraft_admin_token");
    if (savedKey && activeTab === "cms" && !cmsData) {
  ...
  async function fetchCmsData(key: string) {
    setCmsLoading(true);
    setCmsError("");
    try {
      ...
      if (!res.ok) {
        throw new Error("Failed to load CMS fields");
      }

      const resData = await res.json();
      setCmsData(resData.acf || {});
    } catch (err: any) {
  ...
  async function saveCmsData() {
    ...
    try {
      ...
      const resData = await res.json();
      setCmsData(resData.acf || {});
      setCmsSuccess("CMS Settings saved successfully to WordPress!");
          ...
          <div className="flex items-center justify-between bg-[hsl(222,47%,6%)] border border-[hsl(217,32%,17%)] rounded-2xl p-4 shadow-sm">
            ...
            <div className="flex gap-2">
              <button
                ...
                className="px-4 py-2 border border-[hsl(217,32%,17%)] text-xs font-extrabold rounded-xl hover:bg-[hsl(217,32%,17%)]/50 transition-colors disabled:opacity-50"
              >
                Reset Draft
              </button>
              <button
                ...
                onClick={saveCmsData}
                disabled={cmsSaveLoading}
                className="px-4 py-2 bg-blue-600 hover:bg-blue-700 text-xs font-extrabold rounded-xl transition-all shadow-md shadow-blue-500/25 flex items-center gap-1.5 disabled:opacity-50"
              >
                {cmsSaveLoading ? (
