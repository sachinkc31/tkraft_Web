...
export function BeforeAfterSection({ title, subtitle, data }: BeforeAfterSectionProps) {
  const [activeTab, setActiveTab] = useState<number>(0);
  const [sliderPosition, setSliderPosition] = useState<number>(50);

  const transformations = [
    {
      title: data?.before_1_title || "Kitchen Counter Makeover",
      description: "From chaotic clutter to crystal-clear modular jar organization in under 60 seconds.",
      beforeImage: data?.before_1_image || "https://images.unsplash.com/photo-1556911220-e15b29be8c8f?auto=format&fit=crop&w=1200&q=80",
      afterImage: data?.after_1_image || "https://images.unsplash.com/photo-1616486338812-3dadae4b4ace?auto=format&fit=crop&w=1200&q=80",
    ...
    {
      title: data?.before_2_title || "Drill-Free Bathroom Space",
      description: "No noisy power drills or cracked tiles. Mount heavy-duty shelves using lab-tested acrylic pads.",
      beforeImage: data?.before_2_image || "https://images.unsplash.com/photo-1584622650111-993a426fbf0a?auto=format&fit=crop&w=1200&q=80",
      afterImage: data?.after_2_image || "https://images.unsplash.com/photo-1595428774223-ef52624120d2?auto=format&fit=crop&w=1200&q=80",
    ...
    {
      title: data?.before_3_title || "Wardrobe Drawer Transformation",
      description: "Stop hunting for socks and undergarments. Modular PET dividers keep every item visible.",
      beforeImage: data?.before_3_image || "https://images.unsplash.com/photo-1595428774223-ef52624120d2?auto=format&fit=crop&w=1200&q=80",
      afterImage: data?.after_3_image || "https://images.unsplash.com/photo-1595428774223-ef52624120d2?auto=format&fit=crop&w=1200&q=80",
  ...
  const current = transformations[activeTab] || transformations[0];
      ...
      <div className="container">
        ...
        <div className="text-center max-w-2xl mx-auto mb-10">
          ...
          <h2 className="text-2xl md:text-3xl font-display font-extrabold text-[hsl(var(--color-text))]">
            {title || "See The TKraft Difference"}
          </h2>
          <p className="text-xs md:text-sm text-[hsl(var(--color-text-muted))] mt-1">
            {subtitle || "Drag the interactive slider below to see how our drill-free organizers instantly eliminate household clutter."}
          </p>
